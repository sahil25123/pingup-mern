...
import { addMessage, fetchMessages, resetMessages } from '../features/messges/messageSlice'
import toast from 'react-hot-toast'
import api from '../api/axios'
import { ImageIcon, SendHorizonal } from 'lucide-react'

function ChatBox() {
  // Fix: Access messages correctly from Redux state
          ...
          {messages && messages.length > 0 ? (
            messages
              ...
              .map((message, index) => (
                <div 
                  key={index} 
                  className={`flex flex-col ${message.from_user_id === user?._id ? "items-end" : "items-start"}`}
                >
                  <div className={`p-2 text-sm max-w-sm bg-white text-slate-700 rounded-lg shadow ${
                    message.from_user_id === user?._id ? "rounded-br-none" : "rounded-bl-none"
                  }`}>
